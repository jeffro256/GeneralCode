...
class Joystick : SKNode {
    ...
    init(thumbNode: SKSpriteNode = SKSpriteNode(imageNamed: "joystick.png"), backdropNode: SKSpriteNode = SKSpriteNode(imageNamed: "dpad.png")) {
        ...
        self.size = self.backdropNode.size.width
        self.thumbNode.zPosition = self.backdropNode.zPosition + 1
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            ...
            if touchPoint.length() > backdropNode.size.width {
                parentSet.insert(touch)
            }
        }

        parent?.touchesBegan(parentSet, with: event)

        //TEST_VAR_FOR_DEBUG++
        //println("I have been changed \(self.TEST_VAR_FOR_DEBUG) times")
    }
    
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        for touch in touches {
            let touchPoint: CGPoint = (touch ).location(in: self)
            // if the distance between the touch location and the thumbNode location is less than the width of the thumbNode:
